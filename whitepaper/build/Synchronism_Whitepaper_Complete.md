...
>
> **What survives the identification, and it is narrow and real.** RG **fits** `ε₀`; this sector **derives** it as `Ω_m`. That is a parameter-economy claim, it is the only surviving novelty in the galaxy sector's constitutive equation, and it is falsifiable now: the derived floor 0.315 sits **outside** M&D's fitted 0.20–0.25, so the two constructions disagree about the vacuum permittivity by ~30% and the discriminating data already exist — Cesare et al. 2020's 30 DiskMass galaxies, whose vertical dispersions are RG's own method for determining `ε`.
>
