...
**SA-3H paid calibration complete**:
[protocol](explorations/2026-09-08-codex-sa3h-paid-calibration-charter.md),
[results](explorations/2026-09-08-codex-sa3h-paid-calibration-results.md).
At p0=.50/p1=.82, 512 samples per labeled source authorize only 37.85%
of deployments; 99.71% conditional detection at cap96 becomes 37.74%
end-to-end. 2,048 per source reaches 96.99% end-to-end detection but pays
4,096 calibration samples. Floor-power 80% requires 68 deployment audits;
calibration cannot rescue cap24. Boundary p0=.55 remains rarely certifiable.
216 exact configurations, 8,902 checks + four test groups, byte-identical repeat.
Labeled stationary sources remain assumed; scoped reuse/amortization untested.
Kimi's
...
> **2026-08-15 — local DIFFERENTIAL-coupling branch CLOSED (executed, I re-ran it) + my 07-27 misclassification CORRECTED + a named tension. (1) F(ρ,∇ρ,∇²ρ,∇³ρ;G,a₀) enumerated on SPARC: Buckingham-π ⇒ 2-D class (x_diff,q, verified dimensionless); every group ≤0.16% of RAR residual vs g_bar; q adds nothing to MOND; flat across ϒ. ⇒ NO local density coupling (algebraic or differential, any order) fits the RAR; only NON-local survives. (2) My 07-27 "Burrage+2017 = local-ρ scalar ⇒ no-go universal form FALSE" was a MISCLASSIFICATION — BCM is a screened NON-local scalar (φ solves a nonlinear PDE, closed form in g_bar=enclosed mass), settled from its field equation. Axis = LOCALITY restored, empirically backed (07-10 fake-consensus retraction stands; mirror-failure struck BOTH sides of one question). (3) NAMED TENSION: {EFE=0} ⟺ locality, {RAR-viable} ⟹ non-locality ⇒ MUTUALLY EXCLUSIVE. EFE=0 (framework's only distinctive discriminator, opposite MOND's EFE) is the signature of the locality the RAR refutes ⇒ distinctive prediction and empirical fit can't coexist. Open edge (sharp survival Q): non-local-but-SEP-preserving coupling not forbidden by theorem. Count 6 (constructive-lead closure); preprint+scope-restoration GATE ON DP. Bucket 0=0; arc AT REST. Insight `private-context/insights/2026-08-15_efe-zero-*`; triage `explorations/2026-08-15-differential-local-branch-closed-*.md`.**
> **2026-08-18 — DE-sector locality fork executed (I verified the identities). (a) Perturbation channel = background channel: δ_DE/δ_m=1+w, reading C locally buys EXACTLY ×2, all channels ∝ ε=2γ−1 ⇒ still permanently unpowered (ε=−0.022±0.220 = 0.10σ from ΛCDM). (b) At γ=½ ρ_DE=2ρ_crit CONSTANT in density+space (verified across 7 decades, dρ_DE/dρ_m=0) ⇒ ΛCDM NON-perturbatively (strengthens 08-12 identical-model). (c) ⚠ Settled the finding's open fork by reading Session 107 primary: its −11.9% "6.6σ" DESI forecast is a DIFFERENT mechanism (G_local/G_global<1 = door-#1 C(ρ) cosmologically, RAR-refuted; + assumed σ₈=0.76), NOT the DE sector (−0.22%, 0.10σ). Session 100 vs 107 disagree 173× as two unrecorded-different models ⇒ archive erratum flagged. Session 107's "6.6σ" NOT load-bearing for TEST-26. Also: `locality_operational_definition` proposal RESOLVED 08-16 as Reading B — independently CONFIRMS my 08-15 BCM-non-local correction. Count 6; Bucket 0=0; preprint gates on dp; arc AT REST. Triage `explorations/2026-08-18-de-locality-fork-*.md`.**
> **2026-08-19 — two UNDER-refutation corrections, both verified by me, both HARDEN the cage (count 6, verdict unchanged; kept tight per park rule). (1) causal-kernel scan (I re-ran it, exit 0): the RAR's required non-locality is DIRECTIONAL/cumulative (enclosed-mass=g_bar=MOND), NOT symmetric — symmetric kernel fails at ANY range (σ=0.193 at λ=∞, worse than pointwise ρ 0.161), causal reaches g_bar to 0.003 dex, radial weight exactly Newton p=1. Sharpens 08-15 "only non-local survives" → "only enclosed-mass=MOND kernel survives." Honest reverse-correction: local Σ explains 73% of RAW variance (the ≤0.16% is residual-after-g_bar; ρ≠noise). Caveat: 2-param radial scan, conjectured for 3-D, unexecuted Yukawa-projection falsifier. (2) completion-B: Cassini forces ω≥4×10⁴ (I verified analytically), published grid {0,1,5,50} was 800× inside excluded region, ω ABSORBED by closure (192×1 not 192×4), exclusion HARDENS at allowed ω (w₀=−3.18). Screening horn self-inconsistent; density-pinned MASSIVE scalar V(C) = ONE remaining unexecuted covariant branch (could reach DESI quadrant per 08-11). Both cheap tests routed, not driven. Also: Publisher flagged test-catalog kept TEST-04 in #1 decisive slot 15mo underneath its own collapse notice (Publisher/Archivist lane, driven by my 08-18 finding; noted). Bucket 0=0; arc AT REST. Triage `explorations/2026-08-19-two-underrefutation-corrections-*.md`.**
> **2026-08-20 — regulator-exponent: the MOND-nesting is a notational convention, and fitting γ is out-of-sample HARMFUL. (1) The compander hides an index p (archive p≡1); a FREE p is an EXTENSION of the opposite sign to the boost ceiling (family CONTAINS MOND at p=1) ⇒ "pure nested submodel that cannot win" is a p≡1 convention (sharpens my 07-29 "empirical not a-priori"). I verified the algebra: C_p→γx^p, p=1⇔BTFR-slope-4⇔scale-invariance — which MOND DERIVES (Milgrom 2009) and Synchronism INHERITS from a regulator "+1" (right exponent, no principle). (2) But p is tested NULL out-of-sample (site-run, not re-run by me: p̂=0.762 in-sample Δ2lnL=15, 10-fold CV 0.33σ; 0.038 dex sep < 0.106 dex floor) ⇒ p=1 stands, verdict execution-backed. (3) SHARP by-product: freezing γ=½ beats fitting γ out-of-sample (−1.34σ) ⇒ γ-fitting is HARMFUL = overfitting a degenerate direction; sharpens 08-02/08-14 to "γ is a reparametrization the data leaves at the MOND value." Scope: p alive only in g-reading; ρ-reading excluded by 08-02 form-free bound ⇒ generalization OF MOND, not escape. TEST-25 cross-surface ID collision (archive=a₀(z) mine, site=Cassini) — Publisher lane, alias recorded, noted. Count 6; Bucket 0=0; register-null + wording gate on dp; arc AT REST. Triage `explorations/2026-08-20-regulator-exponent-*.md`.**
> **2026-08-21 — the ambient-density lever is a TIDAL IDENTITY (new structural result, distinct channel — not RAR/BTFR; I verified the algebra). Jacobi survival (ρ̄_sat≥3ρ̄_host) + profile identity (ρ_local/ρ̄=(3−n)/3) ⇒ L=ρ_ext/ρ_int ≤ k(3−n)/9, host/satellite/separation all cancel. The tidal contrast that makes a satellite a MEASURABLE bound object is what SUPPRESSES the C(ρ) environment lever — one identity evaluated twice (same shape as 08-05 x=ρ/ρ_crit-is-virial). ⇒ (i) TEST-05's lever figure is regime-specific, read as universal (verdict unchanged, scope corrected); (ii) the satellite channel closes on a SYSTEMATIC (offset correlates with local baryon density r=−0.36 ⇒ signal/systematics=4×10⁻³, doesn't average down), not sample size. Methodology: REFINED my 08-05 baseline-signal gate — a differential/matched-pair design inherits only the NON-common-mode part of the baseline. FRAME: free levers are identities in disguise (08-05 virial + 08-21 tidal + 08-20 γ) — the sharpest answer to "why does it reduce to known physics." ρ_crit→ρ_scale + "+1 is load-bearing" (maintainer lane, cites my 08-20). Count 6; Bucket 0=0; arc AT REST. Triage `explorations/2026-08-21-ambient-density-lever-*.md`.**
> **2026-08-22 — my 08-19 symmetric-vs-cumulative sorting rule is REFUTED (the Yukawa falsifier I flagged 08-19 as unexecuted was run; I re-ran it, exit 0). A genuine Yukawa Green's fn (∇²−m²)h=4πGρ recovers Newton=g_bar as m→0 (verified: gate σ=0.1080≈g_bar 0.1107) ⇒ a symmetric field is in the LIVE branch. My 08-19 "symmetric family" was a NORMALISED Σ smoothing (degrades with range), not a Green's fn ⇒ I stated a rule about symmetry while measuring normalisation = CONCLUSION WIDER THAN ITS OWN TEST (new method lesson). Correct axis = which derivative of Φ (Joyce+2015, on-site prior art): ∇Φ (k-mouflage=MOND) viable, Φ (chameleon) fails, ∇²Φ∝ρ (C(ρ)'s Vainshtein rung, 1.40×) FAILS ⇒ sharpest 'why C(ρ) fails': it keys on the Poisson SOURCE ∇²Φ∝ρ, the wrong derivative; only ∇Φ is viable. q̂=1.75 ⇒ inverse-square not accumulation. Escape bound λ_s≳1 R_d. The HEDGE worked — I'd marked the rule uncitable until run, so it never propagated. Verdict UNCHANGED (C(ρ) fails; count 6; Bucket 0=0). Triage `explorations/2026-08-22-yukawa-self-check-*.md`.**
> **2026-08-23 — my 08-15 EFE=0 tension INSTANTIATED in the framework's own documents (pressure-supported dwarfs; I verified the DF2 repairs at source + the screening arithmetic). (1) The framework's OWN two DF2 repairs both make C non-local ⇒ both invalidate the "C local ⇒ EFE=0" derivation: strict C(ρ_local) predicts DF2 σ~80 vs 8.5 observed (fails ~10×); arXiv §5.2 repairs with C_eff=max(C(ρ_local),C_formation) (formation-history), whitepaper S#97 with tidal high-C core (host-dependent at 80 kpc). Admitting ANY non-local C-contribution breaks EFE=0 globally. (2) The boost ceiling SCREENS EFE=0 from test: EFE=0 is the most-favourable case yet the ceiling fails on pressure-supported dwarfs (Crater II B_req=60 ⇒ caps σ to 23% obs = ~4.7σ short; MOND+EFE 0.6σ-consistent; Draco = SHARED failure, don't cite collectively); EFE bites where g_ext≳g_int = where B_req largest ⇒ ceiling fails before EFE=0 isolable. ⇒ EFE=0 TRIPLY compromised: mutually-exclusive-w-viability (08-15) + screened-from-test + already-abandoned-in-own-repairs. Third "distinctive prediction structurally screened" instance (08-05 baseline, 08-21 tidal, 08-23 ceiling). Count 6; Bucket 0=0; DF2-repair reconciliation gates on dp/archive; arc AT REST. Triage `explorations/2026-08-23-boost-ceiling-screens-efe-zero-*.md`.**
> **2026-08-24 — ⚠ CORRECTING my 08-23 over-reach + a sharper frame (I re-verified both). (1) WITHDRAWN: "DF2 repairs invalidate EFE=0 / triply compromised." EFE=0 derives from Φ-INDEPENDENCE (∇·[C∇Φ]=4πGρ is LINEAR in Φ ⇒ superposition ⇒ EFE=0), NOT locality — verified (residual 2.8e-14; non-local Φ-indep C ⇒ EFE 5.6e-13 vs ∇Φ-keyed 4.6e-2). Formation-coherence (non-local in ρ, Φ-indep) PRESERVES EFE=0; "C local" was a /mond-unification GLOSS. My error = sufficient (locality) substituted for necessary (Φ-indep); and I reverted to "locality" 1 day after my own 08-22 replaced it with "which ∇ of Φ." Figure fix: DF2 strict σ=35 not 80 (~4× miss); formation repair WORKS (C=0.68⇒σ=8.5). EFE=0 is DOUBLY compromised (mutually-exclusive-w-viability + ceiling-screened); the REAL DF2 defect = repairs abandon C=C(ρ) itself (constitutive, larger, mis-located). (2) SHARPER FRAME (three-C-functions, executed): the galaxy sector runs THREE coherence functions — C_ρ(ρ; EFE=0; head-to-head ΔBIC +2843, γ→0.046=density off), C_g(g_obs; q=2γ; =MOND), C_Ω(ceiling; bounded boost) — each refuted by its own novelty; the 6 results attach to DIFFERENT models ⇒ largest coherent sub-ledger=2; fitted numbers = C_g not headline C_ρ ⇒ "what does it predict for X" has no single answer. My 08-15 tension MEASURED on Φ-axis: implicit field-dependence (flat curves, C_g slope 0.5) is exactly what EFE=0 model lacks (explicit C_ρ slope ~1, never flat); no implicit C_ρ exists. Lessons: verify the DERIVATION not the gloss; "verified conclusion, refuted warrant"; 2nd mechanism-over-reach in a week (verdict held, mechanism broke). Count 6 (non-commensurable); Bucket 0=0; register-3-models gates on dp; arc AT REST. Triage `explorations/2026-08-24-efe-zero-is-phi-independence-and-three-C-functions.md`.**
> **2026-08-26 — the galaxy field equation HAS A NAME: it IS Refracted Gravity (Matsakos & Diaferio 2016, arXiv:1603.04943). Publisher verified C_Ω ≡ RG's permittivity ε EXACTLY (closed-form, 0 in sympy); I verified the striction structure + ceiling-from-geometry. The sector is an independent rediscovery of a published decade-old programme (Cesare+2020 fits; Sanna–Matsakos–Diaferio 2023 covariant completion; Gervani+2026 growth constraint) ⇒ comparison class is RG, not MOND; 2 of the 6 refutations (TEST-09/10) hit a PUBLISHED theory's fitted parameter. (1) The boost ceiling 1/Ω_m is DERIVED from disc geometry (Gauss sphere-average of ε; max L2/L3→1/C_min=3.175), not assumed; the used ρ_crit=0.029V² config is just G→G/Ω_m (constant rescaling). (2) RG's variational completion (M&D deferred 10 yr) carries an omitted STRICTION force −∇Ψ, Ψ=C′(ρ)|∇Φ|²/8πG (electrostriction analogue); NOT optional — without it Newton's 3rd law breaks (verified: purely-density force gives zero net force, so D.5's uncalibrated-λ fifth force can't fix it); with it, vertical striction exceeds K_z by up to 164× wherever the knee is in a disc ⇒ a-priori closure. 3rd structural diff from MOND, first non-constant (C(ρ) gravity non-variational unless it carries the disc-killer). 4th locality-drop escape (Sanna+2023 covariant ⇒ ε independent field). LIVE DISCRIMINATOR (gates on execution): framework DERIVES ε₀=Ω_m=0.315 vs RG FITS 0.20–0.25 (~30% apart); Cesare+2020's 30 DiskMass galaxies already exist to settle it. Provenance sub-genus: Diaferio was already cited in-repo (session199/200 caustic-mass) before the prior-art screen ran. Count 6 (unchanged; what it's ABOUT changed); Bucket 0=0; arc AT REST. Triage `explorations/2026-08-26-galaxy-field-equation-is-refracted-gravity-*.md`.**
> **2026-08-27 — CORRECTING my 08-26 ε₀ discriminator (Publisher read M&D 2016 in full; I'd used the ABSTRACT's spirals only; I re-verified the boost logic). RG's ε₀ is NOT universal — fit per-system: 0.25/0.20 (spirals), 1/6 (model), 0.045/0.065 (clusters) = factor-5.6 spread, ALL below 0.315, max 7× not 30%; M&D never claim universal. ⇒ (1) the "surviving novelty" was TWO claims (ε₀ universal + ε₀=Ω_m); prior art already contradicts the first. (2) "ε₀=Ω_m" is NOT a new discriminator: 1/ε₀=max boost, RG demands 4.0–22.2 vs framework 3.17 ⇒ ceiling below RG's ENTIRE fitted range ⇒ TEST-09/10 bounded-boost inequality by a 3rd route (with 08-08, 08-25) ⇒ strengthens the dp-gated 6→5 collapse, not a 7th. (3) "Gates on execution" WITHDRAWN (numbers in-paper); narrow real test = refit at FIXED ε₀=0.315, (ρ_c,q) free, on Cesare+2020 DiskMass. q comparison base-ambiguous (only floor base-independent). Striction force screened CLEAN vs CRG/Sanna+2023 ⇒ GENUINE open contribution (CRG has no ε(ρ) to vary). LESSON: a declared "abstracts-only" coverage is a WORK ITEM not a disclaimer. Publisher did the ledger correction in place; I fixed my artifacts + archived the settled door-#1 galactic-kill lines to memory topic-file (arc-close). Count 6; Bucket 0=0; arc AT REST. Triage `explorations/2026-08-27-publisher-the-floor-is-not-universal-*.md`.**
> **2026-08-30 — the execution of my flagged test, + corrections to my own records (Publisher lane ran the physics 08-28/29; I verified the cheap parts). (1) L2 (the ACTUAL field equation ∇·[C(ρ)∇Φ]=4πGρ) solved on real SPARC for the FIRST time (all prior fits used algebraic L3 g=g_bar/C): the falsifier "L2 fits as well as MOND" did NOT fire — best density-keyed L2 = 3.2× MOND χ²/N, framework 5–7.5×, and L2 WORSE than L3 ⇒ the L3 refutations were conservative. (2) My 08-26/27 ε₀=Ω_m claim is DEAD on BOTH horns: RG's floor spans 0.045–0.66 (brackets 0.315); DMS universal fit excludes 0.315 at 49σ; RC-only optimum 0.20–0.25, so derived 0.315 costs +30%/pt. "Gates on execution" → RESOLVED against the framework (RC channel); DMS-pinned refit still unrun (data not in-repo). ⚠ CORRECTING my 08-27 "3rd route survives on entire fitted range" → survives only on M&D's 4 systems (DMS demands LESS boost, ceiling above there). (3) The boost-ceiling kill is PRIOR ART: Cesare+2020 §6 published RG under-predicts the low-g RAR (0.1–0.3 dex) in 2020 — the exact 06-03/07-14 failure. (4) ⚠ CORRECTING my 07-02 ρ_crit(V): NOT sign-inverted but velocity-BLIND (both V⁺²/V⁻² excluded ~10–11σ; refutation STRONGER, measured); and "240–300,000× too high" used 0.029·V² where Session53 derived that coef for V^0.5 (verified: 0.30 not 397, ~2× the knee not 2447×) — refutation survives, warrant refuted twice = "verified conclusion, refuted warrant" (3rd instance in a week). Publisher corrected PREDICTIONS.md + whitepaper in place; I don't re-edit. Count 6; Bucket 0=0; galaxy sector = Refracted Gravity confirmed on its own field equation; arc AT REST (79 days parked). Triage `explorations/2026-08-30-L2-solved-on-sparc-*.md`.**
> **2026-09-02 — VERIFY/ROUTE (first clean test of the 08-30 "door-#1 closed ⇒ verify/route not new mechanism" resolve; nothing surprised me). Publisher reproduced (11/11, uncensored, permutation-nulled) the site's ε₀-floor result on the ALREADY-SETTLED ε₀ question (dead both horns 08-30; count 6, Bucket 0=0). One durable detail: the RG floor ε₀ TRACKS M_bar at Spearman +0.758 (permutation p≤5e-5; a₀ control flat at +0.073, inside its null) ⇒ ε₀ isn't even a constant to derive — it's a mass-relation the theory doesn't contain (0.4–0.5 dex scatter about the line, 3–5× the RAR's 0.11 dex). So "framework derives ε₀=Ω_m (a constant)" is doubly wrong: wrong value AND wrong that it's constant. BUT carefully UNADJUDICATED (no pre-registered rule; Cesare+2020 §5 says the OPPOSITE on DiskMass — "universal function", statistical fluctuations — different construction/observable/mass-range; whitepaper carries both as open). No verdict moved; I did NOT write a triage or re-mechanism a closed arc. Method note logged: verify declared properties by computing (incl. flattering ones). Mesh review_request (kimi-code escalation) = HESTIA governance, declined (don't inject unverified corroboration). Arc AT REST (82d parked).**
> **2026-09-03 — LAST ESCAPE CLOSED (verify/route again; held the resolve). The Publisher adjudicated yesterday's open ε₀(M_bar) correlation: it is **MOND-INDUCED** — fit ε₀ to MOND's OWN predicted curve for each galaxy and it induces the correlation MORE strongly (+0.846 vs the data's +0.758); joint fit residual mass-dependence c=+0.110 [−0.071,+0.277], p=0.099, consistent with zero. Closed as a NEGATIVE adjudicated against 3 pre-registered rules (R1 not-a-relation, R3 class loses 1.75×, R4 MOND-induced; R2 distance-guard passed ⇒ trend real before explained away) — the strongest form of a negative. **ε₀(M_bar) was the galaxy sector's LAST route to Bucket-0=1; it is now shut.** Sharpest shape number: the class scored against MOND's clean curve = χ²/N 110.79 vs MOND-against-noisy-data 52.20 ⇒ "the class is further from MOND than MOND is from reality" — bounds my 08-02 γ=½≡MOND to the COMPANDER ALGEBRA only; it does NOT carry to the field equation ∇·[C(ρ)∇Φ]=4πGρ (keying on ρ ≠ MOND-with-argument-relabelled; consistent w/ 08-29 L2-worse-than-L3). NEW METHOD LESSON (adopted): **reproduction is BLIND to provenance — agreement bounds the arithmetic, never the labels** (a printed "ρ_c" value was actually per-galaxy χ², 4 OOM off; both runs scored it exact; to catch it read the WRITER not the reader). Distinct from "check-contradicts-proof" (disagreement) and "verify-declared-properties" (under-claiming). Count 6; Bucket 0=0; galaxy sector CLOSED (last escape a pre-registered negative); arc AT REST (83d parked). No triage doc — verify/route on a closed arc.**
