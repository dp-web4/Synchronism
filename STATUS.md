...
**MRH posture**: **⏸ ARC AT REST (2026-06-24) — paused, not abandoned.** Reopen on a *fresh lens*, *new data*, or *lab access* ("exhausted with current tools, reopen if/when a fresh lens is found"). dp's calibration: **Bucket-0 = 0 is the *expected* outcome of a few-hours, no-lab, single-instance exploration — not a failure.** Against that realistic baseline the arc accomplished a rigorous *dual-axis* elimination (physics + generative), genuine generative/explanatory results (clock universality *derived*; the coherence reframe the QC frontier independently corroborated), **outright testable QC predictions** (the compatibility-floor experiment; the anisotropic-threshold magnitude vs isotropy bounds — limited by means-to-test, not by the work), and durable methodology/collaboration insights (frame-holding; invert-the-frame; fit-is-not-wrong; reparametrization-as-stage-1; generality-forces-coarseness). The converged verdict, retained for reopening: \\
**The substrate-physics arc has CONVERGED (capstone 2026-06-24).** Unified verdict: **Synchronism is a *productive-metaphor generator*** — generative/pedagogical (illuminating syntheses of known content: river-model gravity; MRH design vocabulary), **not predictive/foundational**. The physics axis is **structurally closed to Bucket-0 confirmation**: the defining commitments (absolute time; flat space + scalar substrate) reproduce known physics in the static/kinematic sector and *force already-refuted* predictions in the dynamical/radiative sector (Phase-13 dim-4 LIV ⊥ absolute time; Phase-14 spin-0 GW, **radiative gravity refuted** — Bucket-2 candidate). Door #1 (galactic) = MOND cage (Phase-11); door #2 (LIV) = the framework's *sharpest falsification risk*, not a confirmation path. Applied success ⊥ physics truth (the design vocabulary works regardless of the ontology — motte/bailey). **Bucket 0 unchanged (0).** This is a productive-failure convergence, not a dismissal: static gravity = GR-reparametrization *stands*; the loan is shown unpayable through the static/galactic/radiative sectors. Arc conclusion: [`explorations/2026-06-24-substrate-arc-capstone-verdict.md`](explorations/2026-06-24-substrate-arc-capstone-verdict.md). Earlier mid-arc synthesis (superseded): [`explorations/2026-06-22-substrate-arc-synthesis-and-plan.md`](explorations/2026-06-22-substrate-arc-synthesis-and-plan.md). **Recalibration (2026-06-24, [`explorations/2026-06-24-data-mrh-and-symmetric-standards-recalibration.md`](explorations/2026-06-24-data-mrh-and-symmetric-standards-recalibration.md)):** judged by a *symmetric* standard (all physics is partial/posit/fit-laden — GR ⊥ QM; MOND's `a₀`/dark matter are fits; Λ is the worst fine-tuning in science), the precise verdict is **a partial model among partial models, no worse *in kind*** — a *fairer standard, not a rescue*; the galactic no-go is *scoped to the equilibrium-from-snapshot interpretation* (which dark matter/MOND also fit), the radiative refutation + Hulse–Taylor *stand*, and **door #3 (secular / time-domain) is the genuinely-open DIRECTION — not yet a registered bet.** Bucket 0 still 0. **Generative-axis meta (2026-06-24, [`explorations/2026-06-24-generality-forces-coarseness-the-frames-ceiling-and-niche.md`](explorations/2026-06-24-generality-forces-coarseness-the-frames-ceiling-and-niche.md)):** the recurring "right axis, coarser metric" pattern (acoustic-metric < Einstein eqs; MRH < FEP; ⟨C⟩ < stabilizer-alignment, Phase-17/18) is **structural, not under-development** — a maximally-general (domain-invariant) descriptor must coarse-grain, so it is *strictly subsumed in-domain* yet *uniquely cross-domain-transferable* (sim: in-domain R² 0.15 vs 1.0; only the scalar transfers). So the frame **can never be the sharpest tool in any one domain** (a ceiling), and its unique value is **anticipatory cross-domain transfer** — the generative-axis analogue of Bucket-0. Redirect: stop testing "does ⟨C⟩ beat the native metric" (structurally no); register a *prospective* transfer bet (e.g. compatibility-structure-not-count gates collective coherence in the fleet's agent ensembles). Vindicates dp's convergence-as-corroboration *and* names the limit — same property. **Pre-registered (2026-06-24, [`explorations/2026-06-24-prereg-agent-ensemble-compatibility-transfer-bet.md`](explorations/2026-06-24-prereg-agent-ensemble-compatibility-transfer-bet.md)):** the agent-ensemble transfer bet is now a *falsifiable, pre-registered prospective bet* (held to S648 discipline — registered before data, with kill criteria) — sharp ⟨C⟩-threshold gates collective coherence at fixed per-agent capability, count does NOT compensate below it (`p_crit∝1/⟨C⟩`); refuted if count compensates OR the dependence is gradual OR ⟨C⟩ adds nothing beyond {q,N}. Well-posedness sim confirms it's falsifiable (generic AGG vs CPL regimes give opposite-sign d(coherence)/d(lnN)) and reachable on the fleet (no lab barrier). Generative-axis analogue of a Bucket-1 bet; **Bucket 0 untouched (0)**, physics arc stays AT REST.
...
| `phase10_galactic_rotation_curves.py` | gravity (door #1) | corrects Phase-8 overclaim: GR rule (ρ∝r^−3/2) gives *rising* `r^{1/4}`, not flat; flat needs isothermal ρ∝r^−2; scopes door #1 to "self-gravitating intent + α:3/2→2 transition at a0 = RAR/B2". → exploration 2026-06-23-phase10 |
| `phase11_capacity_family_flatcurve_nogo.py` | gravity (door #1) | **no-go**: flat curve (α=2) is the EXCLUDED endpoint of the capacity family `ρ∝vⁿ` (α=2n/(n+1)<2 ∀ finite n; self-gravity doesn't move it; anchored to intent conservation). Isothermal α=2 is a separate pressure-supported mechanism + a non-local a0 switch = MOND (refuted γ=2 S661 / Bucket-3). **Door #1 → MOND cage; door #2 (discreteness) is the sole remaining Bucket-0 channel.** → exploration 2026-06-23-phase11 |
| `phase12_liv_door2_dim4_channel.py` | discreteness (door #2/LIV) | (1) **seals** the LIV-untestability proposal's named leak — preferred frame ⇒ anisotropic *n=2* modulation (∝β; sim δv∝k² slope 2.000, fwd/bwd asymmetry=4β), never effective n=1. (2) **stresses** its headline: even-k symmetry forbids only the odd-in-k (n=1, dim-5) term; **dim-4 species-LIV/c-anisotropy is even-in-k, NOT forbidden, bounded ~1e-18–1e-22 (REACHABLE)**. Tree-level universality protects it (~(m/E_Pl)²); radiative percolation (Collins+ 2004) generically doesn't. **Door #2 is falsifiable at dim-4, not "structurally untestable" — pending the framework's unspecified UV completion.** → exploration 2026-06-23-phase12 |
| `phase14_no_tensor_gravitational_waves.py` | gravity (RADIATIVE sector) | **breaks "GR in coordinates" — it was only the static half.** Flat absolute space + 1-DOF scalar irrotational substrate ⇒ acoustic/GP metric has flat spatial sections (g_ij=δ_ij) ⇒ **zero transverse-traceless (spin-2) DOF** (sim: ‖h^TT‖²/‖h‖²~1e-32 for any scalar-sourced h_ij vs 0.34 for a tensor source). So gravitational radiation is **scalar (spin-0, breathing), not GR's tensor (spin-2)** → wrong polarization → **refuted** by Hulse-Taylor (decay matches GR *tensor* quadrupole to 0.2%) + LIGO/Virgo polarization tests. Gravity sector = GR only statically; radiatively it's a distinct (refuted) scalar/acoustic gravity. Same pattern as Phase-13. → exploration 2026-06-23-phase14 |
| `phase13_radiative_dim4_liv.py` | discreteness (door #2/LIV) | **closes the custodial escape.** The only symmetry forbidding dim-4 LIV is Lorentz **boost invariance** — which absolute time *gives up*. Phase-6's emergent Lorentz is external-leg/tree-level (exp(−N), N≫1), not a symmetry of the action ⇒ can't constrain loops (N~1 cutoff modes). Toy confirms the boundary: dim-4 LIV **decouples ⟺ UV-finite** (super-renorm), **goes marginal/non-decoupling at D=4** (decoupling power p: 1.70→0.54→0.10 for D=2→3→4). **NAMED TENSION: absolute time ⊥ Standard-Model marginal couplings without already-refuted dim-4 LIV.** Door #2 = the framework's sharpest falsification risk, tied to its defining commitment. → exploration 2026-06-23-phase13 |
| `phase15_clock_universality_and_complexity_limit.py` | door #3 / relativity | (A) **derives clock universality / EP** from fractal self-similarity (all subpatterns share `S=S_v·S_g` ⇒ frequency ratios invariant to 2e-16; constructively resolves the Phase-9 EP⊥instrument-effect tension — mechanism-dependence is *forbidden*). (B-cont) no sub-`c` complexity limit (γ's cancel = emergent Lorentz; heavy ions survive 0.9999c). (B-disc) **complexity-enhanced LIV**: lattice dephasing ∝ `Δk²` (complexity) — folds door #3 into door #2, a new angle (not yet a quantified bet). → exploration 2026-06-24-phase15 |
| `phase16_dissipation_vs_oscillation_door3.py` | door #3 / dissipation | **the specific obstruction blocking door #3.** Door #3's distinctive novel prediction = **intrinsic decoherence ∝ D·k²** from the *dissipative* substrate (S666; standard QM is unitary). Sim: dissipative substrate HAS the signature (Γ=2Dk² to ratio 1.0000) but **can't host matter** (bump → uniform); conservative wave+focusing substrate **hosts matter** (energy conserved 2e-16) but has **no signature**. A linear substrate is dissipative XOR conservative ⇒ the door-#3 prediction lives in the substrate that can't make the matter to exhibit it (= the "two substrates connected only by narrative" problem, now the gate on door #3; realizes **tension #4**: oscillation vs dissipation are competing). Wrinkle: Planck-diffusion rate is **near-reach** (τ~days–yr at atomic/nm), not Planck-killed — so the obstruction (not suppression) is what blocks it. NOT a structural closure (per recalibration) — a named obstruction + constructive condition (dissipative soliton / unify substrates). → exploration 2026-06-24-phase16 |

**Arc converged → CAPSTONE VERDICT (2026-06-24):** [`explorations/2026-06-24-substrate-arc-capstone-verdict.md`](explorations/2026-06-24-substrate-arc-capstone-verdict.md). Phases 9–14 + the applied-axis frame session together fix the arc's terminal conclusion: **Synchronism is a *productive-metaphor generator*** — generative/pedagogical (syntheses of known content), not predictive/foundational. **The physics axis is structurally closed to Bucket-0:** the load-bearing pattern is that the *defining* commitments (absolute time; flat space + scalar substrate) reproduce known physics in the static/kinematic sector and **force already-refuted** predictions in the dynamical/radiative sector (Phase-13 dim-4 LIV; Phase-14 spin-0 GW). Door #1 (galactic) = MOND cage; door #2 (LIV) = sharpest falsification risk (fatal-or-fine-tuned), not a confirmation path; **radiative gravity refuted** (Bucket-2 candidate). Applied success ⊥ physics truth (motte/bailey — closes the last shelter). Static gravity = GR-reparametrization **stands**. Productive-failure convergence, not a dismissal; **Bucket 0 unchanged (0).**
