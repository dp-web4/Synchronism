...
| **A.3-vs-Session-11/S665/S666 tension** | Appendix A.3 claims exact NS identification (currently tagged `✅` — that tag itself is being retagged); Session 11 finds 1-DOF scalar diffusion; S665/S666 find irrotational + dissipative | All three statements cannot hold at face value. Phase 1 sim resolves which qualifies. |
...
- Saturation reframe accepted as the right structural response to Session 11's scalar-diffusion finding. Old EOS `P = I_max − I` shown broken (negative sound speed). Recommends sufficiency-claim epistemological stance. Specifies obligations: stable EOS, momentum-equation derivation from discrete rules, oscillation demonstration in simulation.
...
| **OQ-fN** | Pattern-reconstruction function `f(N)` derivation from discrete substrate rules. Boundary condition: `f(N) → 1` as `N → 0` | `[ACTIVE-MRH]` | Sequences with Phase 1 sim. Possibly partially addressed in Appendix A.19. |
...
| **OQ-A3-Tension** | Reconcile Appendix A.3's "exact NS identification" with Session 11's 1-DOF scalar diffusion and S665/S666's irrotational + dissipative findings | `[ACTIVE-MRH]` | Documentary surfacing; outcome of OQ-Oscillation will inform the reconciliation. |
