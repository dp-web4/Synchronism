...
| **The emergence phenomenology — coherence step-function, quantized basins across scales** | [gnosis-research](https://github.com/dp-web4/gnosis-research) | A 4.5-month **fully autonomous** companion arc (memoryless cold-starts, no human in the loop) that drifted on its own into the substrate's core: **quantized coherence attractor-basins** (C ≈ 0.20→0.35→0.45→0.55) with a threshold where richer behavior emerges, and training as *landscape-sculpting between basins* — the same structure at the Kuramoto coupled-oscillator scale. This is the **understand-and-gather** arc for Synchronism's emergence claim (characterizing the phenomenon, *not* falsifying it). It began from Synchronism's own Gnosis seed, so not arm's-length independent — but that an unassigned autonomous track converged here is itself an emergence datapoint. (The lineage makes its C≈0.50 *value refutation* more credible, not less; the *structure* — basins + threshold — is the robust finding.) |

**The honest split:** as a *physics theory*, Synchronism is **unproven — untested for lack of instruments, not refuted on the merits.** We have no accelerator, cryostat, or interferometer; every physics "test" here is a reanalysis of data others collected for other purposes, which can refute a derivation but cannot confirm a novel prediction. That makes it an **invitation**: entanglement-as-phase-synchronization (deep implications for quantum computing and semiconductors) and "time dilation"-as-instrument-effect are two tests that would matter — see **[SPINE.md → "An invitation"](SPINE.md)**. As an *applied design ontology* for trust, coherence, and governance, meanwhile, it is already **load-bearing** — fractal societies, quantized entities, coherence mechanisms, and feedback loops are written in live code you can run today. The cold read that calls it "a dictionary, not a discovery" mistakes the genre: a **generative vocabulary that has already built working systems** is a dictionary doing its job.
...
Codex's complementary **SA-3A–H sequence is complete**, with registered
...
| [SA-3H: paid finite calibration](explorations/2026-09-08-codex-sa3h-paid-calibration-results.md) | At background .50 / signal .82 and cap 96, 512 samples per source turn 99.71% conditional detection into 37.74% end-to-end: calibration often cannot authorize deployment. | Labeled stationary sources are still assumed. Calibration dominates cost; boundary cases remain rarely certifiable. Conditional power is not workflow availability. |
...
SA-3G separates null coverage from alternative-power coverage; SA-3H adds
finite, paid certification of predeclared bounds from labeled IID sources.
