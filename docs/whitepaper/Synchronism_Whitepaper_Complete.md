...
> - **It is not a new refutation — it is the bounded-boost inequality again, by a third route.** `1/ε₀` is the maximum boost. RG's fits demand `4.0, 5.0, 6.0, 15.4, 22.2` where this framework's ceiling is `1/Ω_m = 3.17`: the ceiling is below RG's **entire** fitted range, spirals included. That is TEST-09/TEST-10's inequality (`B ≤ 1/Ω_m` binds) sourced from published fits rather than SPARC — a **third independent route** to the standing 6→5 count collapse (with 2026-08-08 and 2026-08-25), not a seventh refutation. Refutation count **HELD at 6**.
>
> **What the discriminator actually is, restated.** The 08-26 form — "a live external discriminator gating on execution" — is withdrawn: the discriminating numbers are printed in the cited paper. What genuinely gates on execution is narrower and worth doing: RG's `ε₀` is fitted *jointly* with `ρ_c` and `q` per system, so a refit at **fixed** `ε₀ = 0.315` with `ρ_c, q` free is the honest test of whether the derived floor survives, and Cesare et al. 2020's 30 DiskMass galaxies are the right sample because vertical dispersions are RG's own method. Stated as a proposal, not a result: nothing has been refit here. *(Verified: `simulations/publisher_20260827_rg_floor_is_not_universal.py`, exit 0 — including the identity `C_Ω ≡ ε` re-checked against the **verbatim** Eq. (4.1) rather than the relayed form, sympy zero in both log bases, numeric max 2.2×10⁻¹⁶.)*
>
