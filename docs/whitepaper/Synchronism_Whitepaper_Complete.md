...
> **Status update (2026-08-01): this prediction has now been engaged by published data, and is disfavoured — for evolving too *slowly*.** Ciocan et al. 2026 (MUSE-DARK III, A&A **709**, L16, arXiv:2604.22613) fit a₀ directly from the radial acceleration relation in redshift bins over 0.33 < z < 1.44 (N = 79), obtaining a₀(z) = a₀(0) + a₁z with **a₁ = 1.59 ± 0.1 × 10⁻¹⁰ m s⁻²**. Verified here against the prediction: for a₀(z) ∝ H(z) the implied low-z slope is a₀(0)·(dE/dz)|₀ = 0.49 × 10⁻¹⁰ (Ω_m = 0.315), so the measured evolution is **3.2× steeper than H(z)**. At z ~ 1 the prediction gives 1.86 against a measured 2.38 (+0.12/−0.10) × 10⁻¹⁰, ≈5σ low; the slope comparison is the more robust statement because it does not depend on the a₀(0) normalisation. **Three caveats travel with this.** (i) Note the test route: Ciocan fits the RAR, *not* the BTFR named above, which is why Milgrom's standing BTFR objection to high-z a₀ tests does not apply to it. (ii) The literature is not self-consistent — Gueorguiev 2024 (arXiv:2409.11425) finds a log₁₀(a₀) redshift slope consistent with zero over 0.5 < z < 2.5, and Milgrom 2017 (arXiv:1703.06110) disfavours a₀ evolving *faster* than this; a₀(z) ∝ H(z) is squeezed from both sides by mutually inconsistent measurements, so neither bound is firm. (iii) Ciocan's systematics are large (a_tot and a_bar from a common forward model; ~0.2 dex gas systematics; ~1.5× the SPARC scatter). **The honest status is engaged and disfavoured, not refuted**, and it is not counted among the framework's refutations. Note also that this result is *worse* for the alternative anchor 2πa₀ ≈ c²(Λ/3)^{1/2}, which predicts no evolution at all: what is embarrassed is the programme of deriving a₀ from cosmology, not this framework specifically. First identified in `synchronism-site/explorer/findings/a0-epoch-branch-A-tested-disfavoured-for-evolving-too-slowly.md` (2026-07-30); the Ciocan abstract and the arithmetic above were re-verified independently before this note was written.
