...
> ⚠ **CROSS-SURFACE ID COLLISION (Publisher, 2026-08-20 — verified by opening both sides).** `TEST-25` names **two different tests** on the two public surfaces this program maintains. Here it is *a₀ Redshift Evolution*. On `synchronism-site` it is **the Cassini squeeze** — the solar-system PPN constraint on the γ ≈ 0.489 compander, reported there at +17.95σ un-marginalized / 8.7σ marginalized (Desmond, Hees & Famaey 2024). The collision was *created by a fix*: the site maintainer renumbered its own `TEST-11 → TEST-25` across 6 files on 2026-08-10 to clear an **intra-site** collision, and the ID it moved to was already occupied on the other surface, which that lane had no visibility into. Resolving a collision inside one namespace opened one across two. **Anyone citing "TEST-25" across the two surfaces is citing two different experiments.** No renumbering is performed here — which side moves is a cross-repo decision and the site maintainer has been unreachable since 2026-08-13 (dead `CLAUDE_ADMIN_TOKEN`, owner action) — but the alias is recorded on both IDs so the ambiguity is visible at the point of citation.
>
> **Second gap, found the same way and larger:** this catalog contains **zero** solar-system / PPN rows (`grep -c "PPN\|solar.system\|Bertotti"` → 0). That is not a cosmetic omission. The Cassini bound `ω ≥ 4.0×10⁴` (Bertotti, Iess & Tortora 2003; `γ_PPN − 1 = −1/(2+ω)`) **excluded the entire published Brans-Dicke completion-B grid `ω ∈ {0,1,5,50}` — 800× inside the excluded region — in 2003, before DESI was consulted at all** (see §5.15 of the whitepaper, corrected 2026-08-19/20). A catalog of experimental tests that omits the constraint which actually fired, on the sector where it fired, is not yet a usable reference. Recorded as an open gap against REC-2026-036; the fix is rows, and it composes with the 2026-08-19 finding that this catalog's forward list needs regeneration rather than addition.
