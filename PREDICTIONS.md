...
|---|-----------|----------------------|----------------------|-------------|
| **B1** | **Observer-relative Bell/CHSH** — a purely *local* substrate, measured only via an observer-pattern's phase-lock to a target (free CHSH setting choice), reproduces quantum correlations without superluminal signaling. | This is the *only* place the single-observer constraint differs structurally from a God's-eye coupled-oscillator model. The one seam where a novel result can live. | Run the harness ([`simulations/kuramoto-lattice-suite/`](simulations/kuramoto-lattice-suite/)). **Refuted if** observer-relative statistics obey the CHSH bound S ≤ 2 (local realism holds → no quantum violation) OR if violation only appears with superluminal signaling / superdeterministic setting correlation. | **Likely refuted.** Local substrates generally can't violate Bell without superdeterminism. A clean refutation is itself a *productive* result (it bounds Synchronism to "ontology, not local-hidden-variable physics"). |
